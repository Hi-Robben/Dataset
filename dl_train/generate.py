...
class DataGenerator(keras.utils.Sequence):
    ...
    def __init__(self, datas, batch_size=100, shuffle=True):
        self.batch_size = batch_size
        self.datas = datas
        self.indexes = np.arange(len(self.datas))
        self.shuffle = shuffle
        self.x_train, self.y_train = self.data_generation(batch_datas= datas)
    ...
    def data_generation(self, batch_datas):
        ...
        for i, data in enumerate(batch_datas):
            ...
            image = np.load(data, mmap_mode='r')
            img = list(image)
            images.append(img)
            # print(image.shape)
        ...
        # x = np.array(images)
        # y = np.array(labels)
        # self.x_train = np.array(images)/256.
